...
npm run test:receipts   # 30 — photos, what is sent to the model, what it may say back
```
...
## Security notes
\nThe full Internet-facing threat model, findings, deployment requirements and verification evidence live in [the security and infrastructure audit](docs/SECURITY-AUDIT.md).

- **Passwords**: scrypt, N=32768, r=8, p=3, per-password salt. Cost parameters are stored in the
...
- **Response headers**: a content policy, `frame-ancestors 'none'` and `X-Frame-Options`
  ...
  the path, and a referrer carries the whole path. The content policy is nonce-based and is generated by `src/proxy.ts` for HTML routes;
  API responses still receive the static hardening headers from `next.config.ts`. An
